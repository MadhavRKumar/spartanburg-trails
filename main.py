...
def calculate_intersection_percent(gdf_polygon, gdf_polyline):
    ...
    def calculate_percentage(polygon):
        # clip the polyline to the polygon and calculate the length of the intersection
        clipped_polyline = gdf_polyline.clip(polygon)
        intersecting_length = clipped_polyline.geometry.length.sum()
